...
def corrida(vantagem, vlc_tartaruga, vlc_lebre):
    """A tartaruga e a lebre vão apostar uma corrida. A lebre concede à
        ...
        A tartaruga corre a uma velocidade em metros por segundo (vlc_tartaruga)
    ...
    metros_leb = 0
    segundos = 0
    if vlc_tartaruga > vlc_lebre:
        return -1
    while metros_leb < metros_tar:
        metros_leb += vlc_lebre
        metros_tar += vlc_tartaruga
        segundos += 1
        if metros_leb == metros_tar:
            segundos += 1
    return segundos
...
def compra(aplicacao, valor_carro):
    ...
    while aplicacao < valor_carro:
        aplicacao = (aplicacao * 0.007) + aplicacao
        valor_carro = (valor_carro * 0.003) + valor_carro
        meses += 1
    return meses
...
def populacao_dodo(populacao):
    ...
    """
    populacao_inicio = 1000000 * 0.1
    ano = 1600
    while populacao > populacao_inicio:
        ano += 1
        populacao -= populacao * 0.06
        populacao += populacao * 0.01
    return ano
